...
const getSlotsForDate = (year, month, day) => {
  ...
  if (weekday > 0 && weekday < 6) return weekdaySlots;
  if (weekday === 6) return saturdaySlots;
  return [];
};
...
const renderCalendar = () => {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    dayButton.addEventListener("click", () => {
      ...
      if (selectedDateText) selectedDateText.textContent = formatReadableDate(currentYear, currentMonth, day);
...
if (bookingForm) {
  bookingForm.addEventListener("submit", (event) => {
    ...
    if (bookingMessage) {
      bookingMessage.textContent = `${nameValue}, your ${serviceValue} appointment is booked for ${formatReadableDate(selectedDate.year, selectedDate.month, selectedDate.day)} at ${timeValue}`;
